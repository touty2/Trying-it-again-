...
const OMIT_PATTERNS: RegExp[] = [
  ...
  /^abbr\. for\b/i,
  /^abbr for\b/i,
  // Purely technical / uncommon proper nouns that add no learner value
  /^used in\b/i,
  /^(a )?place name\b/i,
...
/**
 * Strip trailing parenthetical annotations that add noise without adding meaning,
 * e.g. "to go (literary)" → "to go", "to dwell (old)" → "to dwell".
 * Only strips if the annotation is at the end of the string.
 */
