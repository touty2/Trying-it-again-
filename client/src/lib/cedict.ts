...
const HIGH_FREQ_MULTI = new Set([
  ...
  '差不多','说不定','说不清','看不见','听不懂',
  '做不到','来不及','来得及','用不着','用得着',
  // ── Time expressions (major segmentation error source) ─────────────────────
  '早上','晚上','上午','下午','中午','深夜','傍晚','凌晨',
  '每周','每周末','今晚','今早',
  // ── Technology / everyday objects ────────────────────────────────────────────
  '手机','手表','手术','手段','手续',
  '电话','电脑','电视','电影','电梯','电子','电池',
...
export function cedictSegment(text: string, _version?: number): CedictSegment[] {
  ...
  while (i < text.length) {
    ...
    // ── Step 3: Score candidates with tri-gram look-ahead ──────────────────
    ...
    for (const c of candidates) {
      ...
      // Tri-gram look-ahead: score the next TWO words to prefer splits that
      // leave both following words as clean dictionary matches.
      // e.g. 今天天气 → 今天 + 天气 beats 今天天 + 气 because 天气 scores well
      if (score > 1 && i + c.len < text.length) {
        ...
        for (let nlen = Math.min(maxLen, text.length - nextPos); nlen >= 1; nlen--) {
          ...
          if (CEDICT.has(nword)) {
            ...
            if (ns > nextBestScore) { nextBestScore = ns; nextBestLen = nlen; }
            break;
          }
        }
        // Look one step further (word after next)
        let nextNextScore = 0;
        if (nextBestLen > 0 && nextPos + nextBestLen < text.length) {
          const nnPos = nextPos + nextBestLen;
          for (let nlen = Math.min(maxLen, text.length - nnPos); nlen >= 1; nlen--) {
            const nword = text.slice(nnPos, nnPos + nlen);
            if (CEDICT.has(nword)) {
              nextNextScore = scoreCandidate(nword, nlen, nlen);
              break;
            }
          }
        }
        // Weight: next word 40%, word-after-next 15%
        score += Math.min(6, nextBestScore * 0.4 + nextNextScore * 0.15);
      }
