...
function FlashCard({
  ...
  // Keyboard shortcuts:
  //   Space — flip card (first press), or Good/rating-3 (second press after flip)
  //   S     — skip card (defer to end of queue, no SRS change)
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      if (e.code === "Space") {
        ...
        if (!flipped) {
          ...
          handleReview(3); // Good on spacebar after flip
        }
      } else if (e.code === "KeyS") {
        e.preventDefault();
        onSkip();
  ...
  return (
    <div className="w-full max-w-lg mx-auto">
      ...
      <div className="mt-4 flex items-center justify-center gap-3">
        ...
        {/* Mark as learned — small link, not a primary action */}
      ...
      <AnimatePresence>
        {flipped && (
          ...
          >
            ...
            {/* Skip row — defers card to end of queue, no SRS change */}
            <div className="flex justify-center pt-1">
              <button
                data-action-btn
                onClick={onSkip}
                className="flex items-center gap-1.5 px-5 py-2 rounded-lg border border-border/50 bg-transparent hover:bg-muted/40 text-muted-foreground hover:text-foreground text-xs font-medium transition-all duration-150 active:scale-[0.97]"
